...
public class Helper {
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			int vmType = i / (int) Math.ceil((double) vmsNumber / org.cloudbus.cloudsim.examples.power.Constants.VM_TYPES);
			vms.add(new PowerVm(
					i,
					brokerId,
					org.cloudbus.cloudsim.examples.power.Constants.VM_MIPS[vmType],
					org.cloudbus.cloudsim.examples.power.Constants.VM_PES[vmType],
					org.cloudbus.cloudsim.examples.power.Constants.VM_RAM[vmType],
					org.cloudbus.cloudsim.examples.power.Constants.VM_BW,
					org.cloudbus.cloudsim.examples.power.Constants.VM_SIZE,
					1,
					"Xen",
					new CloudletSchedulerDynamicWorkload(org.cloudbus.cloudsim.examples.power.Constants.VM_MIPS[vmType], org.cloudbus.cloudsim.examples.power.Constants.VM_PES[vmType]),
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			int hostType = i % org.cloudbus.cloudsim.examples.power.Constants.HOST_TYPES;

			List<Pe> peList = new ArrayList<Pe>();
			for (int j = 0; j < org.cloudbus.cloudsim.examples.power.Constants.HOST_PES[hostType]; j++) {
				peList.add(new Pe(j, new PeProvisionerSimple(org.cloudbus.cloudsim.examples.power.Constants.HOST_MIPS[hostType])));
			}

			hostList.add(new PowerHostUtilizationHistory(
					i,
					new RamProvisionerSimple(org.cloudbus.cloudsim.examples.power.Constants.HOST_RAM[hostType]),
					...
					org.cloudbus.cloudsim.examples.power.Constants.HOST_POWER[hostType]));
		}
		return hostList;
	}
